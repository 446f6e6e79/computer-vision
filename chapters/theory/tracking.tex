...
The filter iterates through the following equations to propagate the state estimate $\hat{x}$ 
and the error covariance matrix $P$:
\paragraph{1. Prediction}
Project the state and uncertainty forward, discarding the optional control input for simplicity:
\[ \hat{x}_k^- = A_k \hat{x}_{k-1} \]
Once we have the predicted state, we also need to predict the error covariance matrix, which represents the uncertainty of our state estimate:
\[ P_k^- = A_k P_{k-1} A_k^T + Q_k \]

\paragraph{2. Update}
The update step incorporates the new measurement $z_k$ to refine the state estimate and reduce uncertainty.
In particular, we compute the Kalman Gain $K_k$, which determines how much we should trust the new measurement relative to our current state estimate. 
Then, we update the state estimate $\hat{x}_k$ and the error covariance matrix $P_k$ accordingly:
\begin{itemize}
    \item \textbf{Kalman Gain:} $K_k = P_k^- H_k^T (H_k P_k^- H_k^T + R_k)^{-1}$
    \item \textbf{State Update:} $\hat{x}_k = \hat{x}_k^- + K_k (z_k - H_k \hat{x}_k^-)$
    \item \textbf{Covariance Update:} $P_k = (I - K_k H_k) P_k^-$
\end{itemize}