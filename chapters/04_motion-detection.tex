...
If we think about this from a practical point of view, this means that we can only detect the motion of the borders of the objects, while the motion of the internal points 
of the objects cannot be detected as they would replace the same intensity values in the image plane, and therefore they would not cause any change in the intensity of the point.
This is the aperture problem, a very well-known issue of the optical flow, which is detailed in the challenges below.
...
\begin{itemize}
    ...
    An example of a feature-based method is the Good Features to Track algorithm, which we will discuss in the next chapter.
\end{itemize}

\subsubsection{Change Detection}
The change detection is a simple approach to motion detection. It takes as input two frames of a video, taken at different time instants $K$ and $K-1$, 
and it computes the difference between the two frames. This can be done by simply following the following steps:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Thresholding}: to filter out the noise, we can apply a threshold to the resulting image. We can define the \textbf{thresholding operation} as:
    \begin{equation}    
...
\begin{itemize}
    ...
    \item \textbf{Changes in the background}: if a background object starts moving, both the real object and its "ghost" (since the background model is not updated) will be detected as moving objects;
...
\end{equation}
Where $B_t$ is the background model at time $t$, $I(t)$ is the current frame, and $\alpha$ is a parameter that controls the update rate of the background model.
\begin{itemize}
...
Instead of relying on a single background model, we can use a Gaussian Model to represent each pixel in the background.
In this approach, we model each pixel as a Gaussian distribution, and we update the parameters $(\mu, \sigma^2)$ of the distribution over time,
...
\paragraph{Improving the Gaussian Average}
...
\end{equation}
Where $M$ is a binary mask that indicates which pixels are classified as background (0) and which pixels are classified as foreground (1).
...
\begin{itemize}
    \item \textbf{Independence of the color channels}: we can assume that each color channel is independent from the others. Therefore,
    the covariance matrix $\Sigma_i$ can be simplified to a diagonal matrix, which reduces the number of parameters to be estimated.
    \item \textbf{Equal variances}: we can also assume that all the channels have the same variance. This way, we can replace the covariance matrix $\Sigma_i$ from the model,
    in favor of a single variance $\sigma_i^2$.
\end{itemize}
