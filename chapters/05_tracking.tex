...
Following motion detection, the resulting binary masks often contain "noise", in the form of fragmented regions, 
...
This process aggregates neighboring foreground pixels into coherent, labeled regions called \textbf{blobs}, which represent the physical objects in the scene.
The final goal is that each blob corresponds to a single moving object, providing a high-level representation that can be easily tracked over time.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{../media/tracking/splitting.png}
    \caption{Example of splitting: a single object is fragmented into multiple blobs.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{../media/tracking/merging.png}
    \caption{Example of merging: multiple objects are merged into a single blob.}
\end{figure}

In both splitting and merging, it is required to analyze the temporal evolution of the blobs to determine whether they represent a single object or multiple 
objects, and to adjust the tracking accordingly. Usually, matching features such as color, shape, or motion patterns can help to resolve ambiguities in these cases.
...
of the objects and predicting potential occlusions. This way, the system can prepare for the occlusion by maintaining the
identity of the involved objects and anticipating their reappearance after the occlusion.

Usually, the system suspends the tracking of the involved objects during the occlusion, and then re-identifies
them once they reappear as separate entities in the scene. No model update should be performed during the occlusion.
Re-identification is usually done by matching features with the pre-occlusion appearance of the objects.
...
\begin{center}
    \begin{minipage}{0.48\textwidth}
        \centering
        \resizebox{\linewidth}{!}{\input{Tikz/tracking/similarity-intersection.tex}}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \resizebox{\linewidth}{!}{\input{Tikz/tracking/similarity-ssd.tex}}
    \end{minipage}
\end{center}
...
with a set of distinctive local \textbf{features}, extracted from the object itself. We should then be able to track these features
over time, and use their movement to infer the movement of the whole object.
...
Considering a set of images $I = \{I_1, I_2, ..., I_n\}$, we can extract a set of features $F_i$ from each image $I_i$.
...
For each pixel in the image $p(x, y)$ we compute the \textbf{structure tensor} $Z_p$:
$$Z_p = \begin{bmatrix}
...
This is basically the Jacobian Matrix of the image gradients, which captures the local intensity variation around the pixel.
The eigenvalues of the matrix, $\lambda_1$ and $\lambda_2$, provide a measure of the local intensity variation around the pixel.
We can identify three cases based on the values of the eigenvalues:
\begin{itemize}
    \item \textbf{Flat region:} if both eigenvalues are small, the pixel is in a flat region with little intensity variation, making it unsuitable for tracking;
    \item \textbf{Edge:} if one eigenvalue is large and the other is small, the pixel is on an edge, which can be tracked but may not be stable due to the lack of variation in one direction (Aperture Problem);
    \item \textbf{Corner:} if both eigenvalues are large, the pixel is in a corner, which is ideal for tracking because it has significant intensity variation in multiple directions.
\end{itemize}
...
\subsubsection{How to track features}
...
However, due to the noise, the equality cannot hold perfectly. Assuming that the motion between frames is small, 
we can find an approximation of the movement of the feature, instead of trying to find the exact displacement vector $d$.
This approach simplifies the problem to finding the displacement vector $d$ that minimizes the difference between the two images:
...
This is simply the sum of squared differences between the pixel intensities in the two frames, within a window centered around the feature point.
...
The two key components that any feature tracker must consider are \textbf{accuracy} and \textbf{robustness}.
\begin{itemize}
...
& \frac{1}{8} \Big( I^{L-1}(2x - 1, 2y) + I^{L-1}(2x + 1, 2y) + I^{L-1}(2x, 2y - 1) + I^{L-1}(2x, 2y + 1) \Big) + \\
& \frac{1}{16} \Big( I^{L-1}(2x - 1, 2y - 1) + I^{L-1}(2x + 1, 2y - 1) + I^{L-1}(2x - 1, 2y + 1) + I^{L-1}(2x + 1, 2y + 1)\Big)
\end{aligned}
\]

The algorithm starts at the top of the pyramid, where the images are smallest, and works its way down to the bottom, where the images are largest and most precise.
The guess $g^L$ is an estimate of the displacement of the feature, i.e. how far it has moved between the two frames, expressed at the resolution of level $L$.
At the top level $L_m$ there is no prior information about the movement, so the initial guess is $g^{L_m} = 0$.

At each level $L$, the incoming guess $g^L$ is first used to \textbf{pre-translate} (warp) the image $I_{i+1}$ by $g^L$.
The Lucas-Kanade method is then applied to this pre-translated image to compute only the \textbf{residual displacement} $d^L$,
the small motion that the guess did not yet account for.
Because $g^L$ already absorbs most of the displacement, $d^L$ is small enough for the optical flow assumption to hold, even when the true motion between frames is large.

The refined estimate is then propagated to the next finer level:
$$g^{L-1} = 2(g^L + d^{L})$$
Where:
\begin{itemize}
    \item $g^L$ is the guess propagated from the coarser level $L$. It is multiplied by 2 to account for the fact that the resolution at level $L$ is half of the resolution at level $L-1$,
    so the displacement needs to be scaled accordingly.
    \item $d^{L}$ is the correction term computed by Lucas-Kanade at level $L$ to fine-tune the estimate.
\end{itemize}
Repeating this until level $0$ yields the final optical flow $d = g^0 + d^0$.

This is what resolves the accuracy--robustness conflict: at coarse levels a large displacement is reduced to a few pixels,
so a small window can track it robustly, while the fine levels recover the precise sub-pixel motion.
...
It tries to estimate the next position of the object (also known as the state of the system) over discrete time steps, 
...
and acceleration along each axis of the tracked object, creating a 6-dimensional state space.
...
\end{itemize}
Take as example the case of a person walking in a scene. 
If no noise is involved, the state transition function $f_t$ would simply predict the next position of the person based on their current velocity and direction of movement. 
Instead, we need to account for the fact that the person's movement might not be perfectly predictable, and that there might be external factors that can affect their trajectory (process noise).
...
\begin{itemize}
    ...
    \item $v_t$ is the observation noise, generated by the sensors and processing algorithms;
...
\end{enumerate}
Thanks to the recursive nature of the algorithm, the posterior distribution at time $t-1$ improves the prediction for time $t$, 
which in turn improves the posterior distribution at time $t$, and so on.

\paragraph{Prediction Step}
Using the state transition function from \ref{eq:state-transition}, the evolution of the system can be described by the conditional probability distribution:
$$p(X_t | X_{t-1})$$
which models the likelihood of transitioning from state $X_{t-1}$ to state $X_t$.

Using the observation function from \ref{eq:observation}, the likelihood of observing the measurement $Z_t$, 
...
When the new observation $Z_t$ becomes available, the likelihood $p(Z_t | X_t)$ is derived from the observation model in \ref{eq:observation}.
...
\begin{itemize}
    \item A measurement of the state of the system (observation, subject to noise);
    \item A prediction of the next state (prior distribution, also subject to noise);
...
\begin{enumerate}
    \item \textbf{Prediction:} since this scalar example has no motion model, the predicted state is simply the previous estimate carried forward unchanged, together with its uncertainty:
    $$\hat{X}_t^- = \hat{X}_{t-1}, \qquad (\sigma_t^-)^2 = \sigma_{t-1}^2$$
    Note that this step does not use the new measurement $Z_t$: it is the belief about the state \emph{before} looking at the observation.

    \item \textbf{Update:} once the measurement $Z_t$ is available, we correct the prediction by adding a displacement towards it:
    $$\hat{X}_t = \hat{X}_t^- + K_t (Z_t - \hat{X}_t^-)$$
    where $K_t$ is the \textbf{Kalman Gain}, which weights the influence of the new measurement
    relative to the predicted state estimate. It is computed as:
    $$K_t = \frac{(\sigma_t^-)^2}{(\sigma_t^-)^2 + \sigma_{z_t}^2}$$

    We can see the updated estimate as a weighted average between the predicted state $\hat{X}_t^-$ and the new measurement $Z_t$,
    where the weights are determined by the relative uncertainties of the two sources of information.

    Finally, we refine the error variance of the state estimate, which decreases as the new measurement is incorporated:
    $$\frac{1}{\sigma_t^2} = \frac{1}{(\sigma_t^-)^2} + \frac{1}{\sigma_{z_t}^2}$$
\end{enumerate}
...
Recall that the Bayesian framework described the state evolution and the observation through generic non-linear functions $f_t$ and $h_t$.
Since the Kalman Filter assumes that these functions are linear and that the noise is additive and Gaussian, each function reduces to a matrix multiplication:
$f_t$ becomes the state transition matrix $A_t$, and $h_t$ becomes the observation matrix $H_t$.
This is what makes the closed-form solution possible and is the reason why the models below are expressed with matrices instead of generic functions.
...
It is important to note that the particles are \textbf{not concentrated around a single point}: they are scattered across the \textbf{regions of high probability} of the state space, and may form several separate clusters at once.
This is what allows the Particle Filter to represent multimodal distributions and track multiple competing hypotheses simultaneously, unlike the Kalman Filter, which is limited to a single unimodal estimate.

The number of particles $N$ is a \textbf{hyperparameter}, fixed in advance and kept constant across all time steps.
...
$$p(x_t | z_{t}) \approx \sum_{i=1}^N w_{t-1}^{(i)} \delta(x_t - x_{t-1}^{(i)})$$
...
\begin{enumerate}
    ...
    \item \textbf{Weighting:} We update the weight of each particle based on the likelihood of the observation given the particle's predicted state:
    ...
    The idea is to treat the normalized weights as a probability distribution over the current particles, and then draw $N$ new particles from this distribution, with replacement.
    Concretely, a particle's weight is the probability of it being picked: particles with a high weight are likely to be drawn several times (producing duplicates at the same state), while particles with a negligible weight are likely never drawn and simply disappear.
    The result is a new set of the same size $N$, now crowded around the high-probability regions, where each particle is reset to a uniform weight of $\frac{1}{N}$.
